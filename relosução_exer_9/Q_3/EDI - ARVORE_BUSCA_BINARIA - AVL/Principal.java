...
public class Principal {
	public static void main (String[] args) {
		...
		TInfo item = new TInfo(45, "João", "Febre Alta");
		T1.insere(item);
		item.prioridade = 20; item.nome = "Maria"; item.sintomas = "Dor de peito";
		T1.insere(item);
		item.prioridade = 60; item.nome = "Pedro"; item.sintomas = "tontura";
		T1.insere(item);
		item.prioridade = 10; item.nome = "Ana"; item.sintomas = "Falta de ar";
		T1.insere(item);
		item.prioridade = 30; item.nome = "Lucas"; item.sintomas = "Dores Abdominais";
		T1.insere(item);
		item.prioridade = 50; item.nome = "Carla"; item.sintomas = "Infecção";
		T1.insere(item);
		item.prioridade = 70; item.nome = "Rita"; item.sintomas = "Gripe";
		T1.insere(item);
		item.prioridade = 5; item.nome = "Paulo"; item.sintomas = "Desmaio";
		T1.insere(item);
		T1.mostrarArvorePrioridade();
	}

}

/*ystem.out.print("========INSERINDO========\n");
		...
		item.chave = 10; item.nome = "Andrea";
		T1.insere(item);
		T1.mostraArvore();

		System.out.print("Inserindo a chave 30\n");
		item.chave = 30; item.nome = "Amanda";
		T1.insere(item);
		T1.mostraArvore();
		...
		item.chave = 35; item.nome = "Osvaldo";
		T1.insere(item);		
		T1.mostraArvore();

		System.out.print("Inserindo a chave 85\n");
		item.chave = 85; item.nome = "Denise";
		T1.insere(item);		
		T1.mostraArvore();

		System.out.print("Inserindo a chave 8\n");
		item.chave = 8; item.nome = "Juninho";
		T1.insere(item);		
    	T1.mostraArvore();
		...
		System.out.print("Elemento nóo existe\n");
		item.chave = 250;
		T1.Remove(item);
		T1.mostraArvore();
		...
		System.out.print("Nó com filhos para a esquerda e direira\n");
		item.chave = 10;
		T1.Remove(item);
		T1.mostraArvore();
		...
		T1.T = null;
		System.out.print("INSERINDO ITENS (1,2,3,4,5,6,7) NA ARVORE\n");
		System.out.print("-------------------------------------------\n");
